/**
 * WORK-031 — Claude adapter fixture browser E2E.
 *
 * Proves the REAL extension + REAL Claude adapter code (claude-bridge content
 * script + claude-page-runtime + ClaudeProviderAdapter) against LOCAL fixtures
 * (WORK-031: the CODING-AGENT surface at /code is the implementation
 * target; a Chat-only page must BLOCK with no fallback) that reproduces the OBSERVED claude.ai DOM (2026-08-24). No live
 * Deterministic local fixtures — no live Claude dependency:
 *
 *   external execution (provider=claude)
 *     → Open with Companion
 *     → adapter opens the fixture (fixture origin staged in storage.session)
 *     → provider detected as Claude
 ...
 * and the popup Claude session view.
 */
...
const FIXTURE_DIR = join(HERE, '..', '..', '..', 'extension', 'tests', 'claude', 'fixture');
const FIXTURE_PORT = 3779;
const FIXTURE_ORIGIN = `http://127.0.0.1:${FIXTURE_PORT}`;

const API_KEY = 'raw-key-work031-browser-e2e';
...
async function startFixtureServer(): Promise<void> {
  fixtureServer = createServer(async (req, res) => {
    ...
    // WORK-031: the CODING fixture lives at /code/ — map the surface root
    // to code.html and resolve its relative script path.
    if (/^\/code\/claude-code-agent\.js$/.test(path)) {
      path = '/claude-code-agent.js';
...
test.beforeAll(async () => {
  ...
  process.env.AGENT_API_KEY = 'work031-test-agent-key';

  process.env['WFOS_TEST_WORK031_BROWSER_KEY'] = API_KEY;
  stack = await buildIdentityStack();
  const org = await stack.organizationRepository.create({ name: 'WORK-031 Browser E2E Org' });
  const user = await stack.userRepository.upsertByExternalId({
    externalId: 'work031-browser-user',
  ...
  await stack.apiKeyProvisioner.provision({
    keyId: 'work031-browser-key',
    secretRef: 'WFOS_TEST_WORK031_BROWSER_KEY',
    externalId: 'work031-browser-user',
    label: 'WORK-030 Browser Key',
    rawKey: API_KEY,
  });
  project = await stack.projectRepository.create({ organizationId: org.id, name: 'WORK-031 Project' });
  await stack.projectAccessRepository.grant({ userId: user.id, projectId: project.id, roleId: 'owner' });

  const arch = await stack.architectureRepository.create({ projectId: project.id, name: 'W31 Arch' });
  ...
  const req = await stack.requirementRepository.create({
    ...
    title: 'Claude adapter loop works',
  ...
  const crit = await stack.acceptanceCriterionRepository.create({
    ...
    description: 'Execution completes via the Claude adapter',
...
async function createReadyWorkItem(label: string) {
  const wi = await stack.workItemRepository.create({
    ...
    workItemId: label,
    title: label,
    objective: 'Prove the Claude adapter loop',
    scope: 'Claude adapter end-to-end',
  });
  ...
  await workflowEngine.transition({ workItemId: wi.id, toState: 'ready', actor: 'w31-e2e' });
  return wi;
}
...
test.describe('WORKFLOWOS — WORK-031 Claude adapter fixture E2E', () => {
  test('full loop: Open with Companion → Claude detected → prompt injected once → fixture agent → completed', async () => {
    test.setTimeout(180_000);

    const userDataDir = mkdtempSync(join(tmpdir(), 'wfos-claude-'));
    ...
    try {
      ...
      await sw.evaluate((origin: string) => {
        return (globalThis as { chrome?: { storage: { session: { set: (i: Record<string, string>) => Promise<void> } } } }).chrome!.storage.session.set({ 'wfos.claude.fixtureOrigin': origin });
      }, `${FIXTURE_ORIGIN}/code/?xss=1`);

      const page = context.pages()[0] ?? (await context.newPage());
      await loginBrowser(page);

      const wi = await createReadyWorkItem('WORK-CLAUDE-E2E-001');
      await page.goto(`/work-items/${wi.id}`);
      await page.getByRole('button', { name: /Start Implementation/ }).first().click();
      const dialog = page.locator('[role="dialog"]');
      await expect(dialog).toContainText('Start Implementation — WORK-CLAUDE-E2E-001');
      await dialog.getByLabel('External execution').check();
      await dialog.locator('#execution-provider').selectOption('claude');
      ...
      // The adapter opened the FIXTURE in a new tab (Claude DOM reproduced).
        ...
        .poll(
          async () =>
            (await fixturePage!.evaluate(
              () => (window as unknown as { __claudeCodeFixture?: { submits: number } }).__claudeCodeFixture?.submits ?? 0,
            )),
          { timeout: 20_000 },
        )
        .toBe(1);
      ...
      const submitted = await fixturePage!.evaluate(
        () =>
          (window as unknown as { __claudeCodeFixture?: { submittedTexts: string[]; taskPath: string } })
            .__claudeCodeFixture,
      );
      expect(submitted!.submittedTexts[0]).toContain('# Implementation Instructions — WORK-CLAUDE-E2E-001');
      ...
      expect(final?.status, 'execution should complete via the Claude adapter').toBe('completed');
      ...
      // The popup renders the Claude session (work item, provider, conversation).
      const popup = await context.newPage();
      await popup.goto(`chrome-extension://${extensionId}/ui/popup/index.html`);
      await expect(popup.locator('#work-item')).toHaveText('WORK-CLAUDE-E2E-001');
      await expect(popup.locator('#provider')).toHaveText('Claude');
      await expect(popup.locator('#status')).toContainText('completed');
      // Claude Code task URLs are /code/<id> (the fixture mirrors this).
      await expect(popup.locator('#conversation')).toContainText('/code/');
      await popup.close();
    } finally {
      await context.close();
    }
  ...
  test('login wall → BLOCKED "Please sign in to Claude." — no submission, no fake success', async () => {
    test.setTimeout(120_000);

    const userDataDir = mkdtempSync(join(tmpdir(), 'wfos-claude-wall-'));
    const context = await chromium.launchPersistentContext(userDataDir, {
      channel: 'chromium',
      headless: true,
      args: [
    ...
    try {
      ...
      const wi = await createReadyWorkItem('WORK-CLAUDE-WALL-001');
      await page.goto(`/work-items/${wi.id}`);
      await page.getByRole('button', { name: /Start Implementation/ }).first().click();
      const dialog = page.locator('[role="dialog"]');
      await dialog.getByLabel('External execution').check();
      ...
      const popup = await context.newPage();
      await popup.goto(`chrome-extension://${extensionId}/ui/popup/index.html`);
      await expect(popup.locator('#blocked-reason')).toContainText('Please sign in to Claude.', {
        timeout: 10_000,
      });
      await popup.close();
      ...
      if (fixturePage) {
        const submits = await fixturePage.evaluate(
          () =>
            (window as unknown as { __claudeChatFixture?: { submits: number } }).__claudeChatFixture?.submits ?? 0,
        );
  ...
  // ---------------------------------------------------------------------
  test('chat-only surface → BLOCKED "Claude coding environment unavailable or unverified." — no Chat fallback', async () => {
    test.setTimeout(120_000);

    const userDataDir = mkdtempSync(join(tmpdir(), 'wfos-claude-chatonly-'));
    const context = await chromium.launchPersistentContext(userDataDir, {
      channel: 'chromium',
      headless: true,
      args: [
    ...
    try {
      ...
      const wi = await createReadyWorkItem('WORK-CLAUDE-CHAT-001');
      await page.goto(`/work-items/${wi.id}`);
      await page.getByRole('button', { name: /Start Implementation/ }).first().click();
      const dialog = page.locator('[role="dialog"]');
      await dialog.getByLabel('External execution').check();
      ...
      await expect(popup.locator('#blocked-reason')).toContainText(
        'Claude coding environment unavailable or unverified.',
        { timeout: 20_000 },
      );
...
async function loginBrowser(page: import('@playwright/test').Page): Promise<void> {
  const user = await stack.userRepository.upsertByExternalId({
    externalId: 'work031-browser-user',
    displayName: 'WORK-031 Browser User',
  });
