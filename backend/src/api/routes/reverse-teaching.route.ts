 ...
 * HTTP (V2-017 rule 9); the pin resolves from the authoritative V2-002
 * version read + the V2-003 digest — never from client-supplied data.
 *
...
/**
 * The authoritative installed pin + document: the V2-002 visibility-
 * checked version read + the V2-003 parse/digest. null = the reply was
 * already sent (not found / unparseable).
 */
/** The authoritative installed pin + document (the V2-002 read + the V2-003 parse/digest). */
type PinResolution =
  ...
  app.post('/reverse-teaching/sessions', async (req, reply) => {
    return runAuthed(req, async () => {
      ...
      if (
        ...
        invalidRequest(reply, 'workflowId, versionId and installationId are required');
        return reply;
      }
      ...
      const resolved = await resolvePin(
    ...
    return runAuthed(req, async () => {
      ...
      try {
        ...
        const resolved = await resolvePin(
          deps.workflowRepositoryService,
          user.id,
          current.pin.workflowId,
          current.pin.versionId,
          current.pin.installationId,
        );
        if (!resolved.ok) {
          reply.code(resolved.status).send(resolved.body);
          return reply;
