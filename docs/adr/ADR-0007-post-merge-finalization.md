...
3. **Enforcement — the merged-finalization invariant.** A Work Order with merge
   evidence in the repository's first-parent history must be `complete` with
   matching `mergedAs`. Evidence is repository-resident git history in both
   merge shapes this repository actually uses: `Merge pull request #N from …`
   (classic merge commits, bound by PR number) and the architect-merge subject
   convention `WORK-NNN: …` (squash/direct merges, bound by work-order id —
   PR #62 merged this way). The invariant is enforced by the static
   architecture suite against the real git history (CI fails between the merge
   and the finalization — that visible red window IS the enforcement) and
   reported by `governance:status` (`backend/src/development-governance/internal/merged-finalization.ts`
